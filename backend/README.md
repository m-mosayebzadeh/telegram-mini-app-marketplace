...
## ۴.۵. وقتی خطای «no such column» یا «no such table» گرفتی

چون هنوز از ابزار مهاجرت (migration) استفاده نمی‌کنیم، `Base.metadata.create_all` فقط جدول‌هایی که اصلاً وجود ندارن رو می‌سازه — اگه یه مدل تغییر کنه (مثلاً یه ستون جدید اضافه بشه) ولی جدولش از قبل روی فایل `app.db` ساخته شده بود، اون تغییر اعمال نمی‌شه و با خطای SQLite مواجه می‌شی.

چون این مرحله از پروژه هنوز داده‌ی واقعی نداره، ساده‌ترین راه‌حل حذف و ساخت دوباره‌ست:
1. سرور رو متوقف کن (Ctrl+C)
2. فایل `backend/app.db` رو حذف کن
3. دوباره سرور رو بالا بیار — خودکار با شِمای کامل و به‌روز از نو ساخته می‌شه
