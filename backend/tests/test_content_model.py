...
def test_paid_content_must_have_a_spoiler(db):
    db.add(_content(is_paid=True, price_photons=10, has_spoiler=False))
...
def test_paid_content_requires_a_price(db):
    db.add(_content(is_paid=True, price_photons=None, has_spoiler=True))
...
def test_free_content_cannot_have_a_price(db):
    db.add(_content(is_paid=False, price_photons=10))
