...
def _upload(
    ...
    *,
    ...
    if price_photons is not None:
        data["price_photons"] = str(price_photons)
...
def test_paid_upload_forces_spoiler_even_if_client_says_false(client):
    ...
    response = _upload(client, auth, is_paid=True, price_photons=50, has_spoiler=False)
...
def test_paid_upload_without_price_fails(client):
    ...
    response = _upload(client, auth, is_paid=True, price_photons=None)
...
def test_paid_content_without_purchase_returns_402(client):
    ...
    _login(client, 1, "Alice")
    _login(client, 2, "Bob")
    content = _upload(client, auth_a, is_paid=True, price_photons=50).json()

    response = client.get(f"/content/{content['id']}/file", headers=auth_b)

    assert response.status_code == 402
    assert response.json()["detail"]["price_photons"] == 50
...
def test_paid_content_after_purchase_reveals_file(client, db_session):
    ...
    content = _upload(client, auth_a, is_paid=True, price_photons=50).json()
    give_wallet_balance(db_session, bob["id"], amount_toman=50 * settings.photon_to_toman_rate)
...
def test_owner_cannot_purchase_own_paid_content(client):
    auth = _auth_header(1, "Alice")
    _login(client, 1, "Alice")
    content = _upload(client, auth, is_paid=True, price_photons=50).json()
