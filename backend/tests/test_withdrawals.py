...
def give_earnings(client, db, photons):
    """Make user 1 genuinely EARN `photons`, by running a real sale.
    ...
    give_wallet_balance(db, buyer['id'], photons * settings.photon_to_toman_rate)
    offer = _create_offer(client, auth(1), price_photons=photons)
...
def setup(client, db, amount=1_000_000):
    ...
    give_earnings(client, db, amount // settings.photon_to_toman_rate)
    bank = client.post('/wallet/bank-accounts',headers=auth(1),json=BANK)
    assert bank.status_code == 201
    return user['id'], bank.json()['id']

def payload(client, bank, photons=500):
    quote = client.post('/wallet/withdrawals/quote',headers=auth(1),json={'photons':photons}).json()
    return dict(photons=photons, bank_account_id=bank, quote_token=quote['quote_token'], idempotency_key=str(uuid4()))
...
@pytest.mark.parametrize('photons',[0,-1,1.5,'200',True,1_000_000_001])
def test_invalid_amounts(client,photons):
    assert client.post('/wallet/withdrawals/quote',headers=auth(1),json={'photons':photons}).status_code==422
...
def test_owner_checks_and_minimum(client,db_session):
    ...
    q=client.post('/wallet/withdrawals/quote',headers=auth(2),json={'photons':500}).json()
...
def test_stale_quote_reconfirmation_and_historical_rates(client,db_session):
    _,bank=setup(client,db_session)
    body=payload(client,bank)
    update={'photon_to_toman_rate':1200,'chat_commission_percent':10,
...
def test_withdrawal_and_purchase_share_balance_lock(concurrent_client,purchase_kind):
    ...
    if purchase_kind=='content':
        item=_upload(client,auth(2),is_paid=True,price_photons=500).json()
        path=f"/content/{item['id']}/purchase"
    else:
        offer=_create_offer(client,auth(2),price_photons=500)
...
def test_content_purchase_takes_its_commission_immediately(client,db_session):
    ...
    25 photons at the default 5% is 1.25, floored to 1 in the provider's favour."""
    setup(client,db_session)
    client.get('/me',headers=auth(2))
    item=_upload(client,auth(2),is_paid=True,price_photons=25).json()
    ...
    assert tx.commission_photons==1
    assert tx.net_provider_photons==24
    assert tx.commission_toman==1*tx.photon_to_toman_rate
...
def test_historical_pending_transaction_keeps_its_original_fee(client,db_session):
    ...
    setup(client,db_session)
    client.get('/me',headers=auth(2))
    offer=_create_offer(client,auth(2),price_photons=40)
    ...
    tx.commission_photons=4
    tx.net_provider_photons=36
...
def test_fee_is_floored_in_toman(client,db_session):
    ...
    rates.photon_to_toman_rate=2501
    rates.withdrawal_commission_percent=7
    db_session.commit()
    q=client.post('/wallet/withdrawals/quote',headers=auth(1),json={'photons':201}).json()
...
def test_the_quote_reports_the_ceiling(client, db_session):
    ...
    quoted = client.post('/wallet/withdrawals/quote', headers=auth(1), json={'photons': 500}).json()
