...
def _running_chat(client, db_session, *, price_drops=40):
...
def test_a_running_conversation_shows_what_is_reserved(client, db_session):
    ...
    assert row["amount_drops"] == -40  # all of it, until it is known
...
def test_the_provider_sees_money_that_has_not_arrived_yet(client, db_session):
    ...
    assert row["amount_drops"] == 9  # one block of ten, less the commission
...
def test_no_row_ever_names_the_other_person(client, db_session):
    """A list that reads "25 Drops from Sara" is a problem the moment somebody
...
def _sold_content(client, db_session, *, price_drops=20):
...
def test_a_sale_and_a_purchase_are_the_same_event_seen_from_two_sides(client, db_session):
    ...
    assert purchase["amount_drops"] == -20  # what the buyer paid
    assert sale["amount_drops"] == 19  # what the seller kept, after commission
