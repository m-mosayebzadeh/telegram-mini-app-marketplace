...
describe('computePriceBreakdown', () => {
  ...
  it('subtracts the commission from what the provider earns', () => {
    ...
    expect(breakdown.netProviderPhotons).toBe(36)
  ...
  it('rounds a fractional commission in the provider favour', () => {
    // 5% of 25 Photons is 1.25 — the platform takes 1, not 2.
    expect(computePriceBreakdown(25, 2500, 5).netProviderPhotons).toBe(24)
    // 10% of 25 Photons is 2.5 — again floored on the platform's side.
    expect(computePriceBreakdown(25, 2500, 10).netProviderPhotons).toBe(23)
  })
})
